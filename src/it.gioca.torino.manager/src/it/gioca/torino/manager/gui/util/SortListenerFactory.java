...
public class SortListenerFactory implements Listener
{
	...
	private Comparator hourComparator = new Comparator()
    {
        public int compare(Object arg0, Object arg1) {
            ...
            DateFormat df = new SimpleDateFormat(Messages.getString("SortListenerFactory.0")); //$NON-NLS-1$
            ...
            { 
                System.out.println(Messages.getString("SortListenerFactory.1") + v1); //$NON-NLS-1$
                try { d1 = df.parse(Messages.getString("SortListenerFactory.2")); } catch (ParseException e1) {} //$NON-NLS-1$
            }
            ...
            { 
                System.out.println(Messages.getString("SortListenerFactory.3") + v2); //$NON-NLS-1$
                try { d2 = df.parse(Messages.getString("SortListenerFactory.4")); } catch (ParseException e1) {} //$NON-NLS-1$
            }
	...
	private Comparator dateComparator = new Comparator()
    {
        ...
        {    
            ...
            v1.replaceAll("-", "/"); //$NON-NLS-1$ //$NON-NLS-2$
            v2.replaceAll("-", "/"); //$NON-NLS-1$ //$NON-NLS-2$
            
            DateFormat df_europe = new SimpleDateFormat(Messages.getString("SortListenerFactory.9")); //$NON-NLS-1$
//            DateFormat df_usa = new SimpleDateFormat("yyyy/MM/dd");
            ...
            { 
               //System.out.println("[WARNING] v1 " + v1);
                d1 = new Date(Messages.getString("SortListenerFactory.10")); //$NON-NLS-1$
            }
            ...
            { 
                d2 = new Date(Messages.getString("SortListenerFactory.11")); //$NON-NLS-1$
            }
...
//            item.setForeground(items[i].getForeground());
    ...
    private String[] getData(TableItem t)
    {
        ...
        for (int i = t.getImage(0)!=null? 1:0 ;i<colCount;i++)
            s[i] = t.getText(i);
